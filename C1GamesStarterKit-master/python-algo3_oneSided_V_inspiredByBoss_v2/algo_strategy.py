...
if random.uniform(0, 1) <= 0.5:
    place_side_wall_on_left = False
else:
    place_side_wall_on_left = True
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def starter_strategy(self, game_state):
        ...
        #### settings:
        n_trials_place_wall_before_placing_a_factory = 2
        thickness_of_forward_defense = 2
        period_for_upgrade_turrets = 4
        threshold_mobilePoints_for_attack_phase = 20
        upgrade_turrets_randomly = False

        #### attack phase:
        if self.is_forward_defense_complete(game_state, thickness_of_forward_defense):
            attack_phase = False
        if (game_state.get_resource(BITS) >= threshold_mobilePoints_for_attack_phase) or (game_state.turn_number <= 4):
            attack_phase = True
        else:
            attack_phase = False
        if game_state.get_resource(BITS) >= threshold_mobilePoints_for_attack_phase*0.5:
            close_to_attack_phase = True
        else:
            close_to_attack_phase = False

        #### placing wall on the side:
        if game_state.turn_number >= 2 and attack_phase:
            locations = [[15, 3], [16, 4], [17, 5], [18, 6], [19, 7], [20, 8], [21, 9], [22, 10], [23, 11], [12, 2]]
            if place_side_wall_on_left:
                locations = self.flip_locations(locations)
            game_state.attempt_spawn(FILTER, locations)

        #### placing main turrets:
        if game_state.turn_number == 0:
            self.build_Turret_defences(game_state, locations=[[17, 12], [10, 12]])
        elif game_state.turn_number == 1:
            self.build_Turret_defences(game_state, locations=[[24, 12], [3, 12]])
        
        #### placing other defenders:
        if (not attack_phase) and (not close_to_attack_phase):
            wall_coor1_from_left = 0-1
            wall_coor1_from_right = 27+1
        else:
            if not place_side_wall_on_left:
                wall_coor1_from_left = 0-1
                wall_coor1_from_right = 25+1
            else:
                wall_coor1_from_left = 2-1
                wall_coor1_from_right = 27+1
        while True:
            ...
            #### placing forward defenses:
            if game_state.turn_number <= 4:
                n_trials_place_wall = 1
            else:
                n_trials_place_wall = n_trials_place_wall_before_placing_a_factory 
            for trial_index in range(n_trials_place_wall):
                ...
                if wall_coor1_from_left <= wall_coor1_from_right:
                ...
                #### place turret sparsely:
                if game_state.turn_number >= 2:
                    least_busy_score = 1000
                    best_location_turret = None
                    for coordinate1 in range(3,25):
                        for coordinate2 in range(12,12-thickness_of_forward_defense,-1):
                            location_turret = [coordinate1, coordinate2]
                            if game_state.game_map.in_arena_bounds(location_turret) and (not game_state.contains_stationary_unit(location_turret)) and self.is_above_the_V_border(location_turret):
                                busy_score = self.calculate_how_busy_neighborhood_is(game_state, location_turret, neighborhood_size=2)
        ...
        if game_state.turn_number == 1:
            ...
            if attack_phase:
                if game_state.turn_number % 2 == 1:
                    ...
                    deploy_locations = self.filter_blocked_locations(friendly_left_edge, game_state)
                    deploy_index = random.randint(0, len(deploy_locations) - 1)
                    deploy_location = deploy_locations[deploy_index]
                iteration_index = 0
                while True:
                    location_interceptor = [[22, 8]]
                    location_demolisher = [[14, 0]]
                    if place_side_wall_on_left:
