...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def starter_strategy(self, game_state):
        ...
        #### settings:
        random_turret = False
        attack_alternative_directions = True

        #### placing wall:
        if game_state.turn_number >= 2:
            locations = [[16, 4], [17, 5], [18, 6], [19, 7], [20, 8], [21, 9], [22, 10], [23, 11], [11, 3]]
            game_state.attempt_spawn(FILTER, locations)

        #### placing other defenders (factory and turret):
        if game_state.turn_number == 0:
            #### placing factory:
            encryptor_locations = [[13, 2], [12, 3], [14, 2], [15, 3]]
            game_state.attempt_spawn(ENCRYPTOR, encryptor_locations)
            #### placing turret:
            self.build_Turret_defences(game_state, locations=[[17, 12], [10, 12]])
        elif game_state.turn_number == 1:
            #### placing factory:
            encryptor_locations = [[13, 3]]
            game_state.attempt_spawn(ENCRYPTOR, encryptor_locations)
            #### placing turret:
            self.build_Turret_defences(game_state, locations=[[24, 12], [3, 12]])
        else:
            place_factory_index = 0
            while True:
                ...
                if (not can_place_factory) and (not can_place_turret):
                    break
                #### placing turret:
                if can_place_turret:
                    if (place_factory_index >= 2) or (not can_place_factory):
                        place_factory_index = 0
                        for try_index in range(100):
                            if random_turret:
                                location_factory = [random.randint(3,24), random.randint(10,12)]
                                if game_state.game_map.in_arena_bounds(location_factory) and (not game_state.contains_stationary_unit(location_factory)) and self.is_above_the_V_border(location_factory):
                                    game_state.attempt_spawn(DESTRUCTOR, location_factory)
                                    break
                            else:
                                least_busy_score = 1000
                                is_turrent_placed = False
                                for coordinate1 in range(3,25):
                                    for coordinate2 in range(12,10-1,-1):
                                        location_factory = [coordinate1, coordinate2]
                                        if game_state.game_map.in_arena_bounds(location_factory) and (not game_state.contains_stationary_unit(location_factory)) and self.is_above_the_V_border(location_factory):
                                            busy_score = self.calculate_how_busy_neighborhood_is(game_state, location_factory, neighborhood_size=2)
                                            if busy_score < least_busy_score:
                                                least_busy_score = busy_score
                                                best_location_factory = location_factory
                                game_state.attempt_spawn(DESTRUCTOR, best_location_factory)
                                break
                #### placing factory:
                is_factory_placed = False
                if can_place_factory:
                    place_factory_index += 1
                    for coordinate_y in range(2, 10):
                        for coordinate_x in range(22, 5-1, -1):
                            location_turret = [coordinate_x, coordinate_y]
                            if game_state.game_map.in_arena_bounds(location_turret) and (not game_state.contains_stationary_unit(location_turret)) and self.is_above_the_V_border(location_turret):
                                game_state.attempt_spawn(ENCRYPTOR, location_turret)
                                is_factory_placed = True
                            if is_factory_placed:
                                break
                        if is_factory_placed:
                            break
        ...
        if game_state.turn_number == 1:
            ...
            if not attack_alternative_directions:
                iteration_index = 0
                while True:
                    ...
                    if (iteration_index == 0 or iteration_index == 1) and game_state.get_resource(BITS) >= game_state.type_cost(SCRAMBLER)[BITS]:
                        ...
                        if game_state.get_resource(BITS) >= game_state.type_cost(EMP)[BITS]:
                            #### placing demolisher:
                            game_state.attempt_spawn(EMP, [[14, 0]])
                    iteration_index += 1
            else:
                #### random location for deploying from left edge:
                ...
                while True:
                    ...
                    # elif game_state.get_resource(BITS) >= game_state.type_cost(EMP)[BITS]:
                    #     #### placing demolisher:
                    #     if game_state.turn_number % 2 == 0:
                    #         game_state.attempt_spawn(EMP, [[14, 0]])
                    #     else:
